...
public class NotificationDaoImpl extends BaseDaoImpl<Notification, Long>implements NotificationDao {
	...
	public List<Notification> getNotifications(Long userId,int count) {
		String hql = "from Notification where recipientId=? and notificationType in (1,2,3,5,6,7,8,15,16) and status='enable' order by create_at desc";
		Session session = getSessionFactory().getCurrentSession();
	...
	@Override
	public List<Notification> getNotificationsByPage(Long paramLong, int count, Long id, int flag) {
		...
		if(flag == 1){
			...
			String hql = "from Notification where recipientId=? and notificationType in (1,2,3,5,6,7,8,15,16) and create_at <= ? and id != ? and status='enable' order by create_at desc";
			...
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String create_time = sdf.format(new Date(createTime.longValue()));
			String hql = "from Notification where recipientId=? and notificationType in (1,2,3,5,6,7,8,15,16) and create_at >= ? and id != ? and status='enable' order by create_at";
			Query query = session.createQuery(hql).setLong(0,paramLong).setString(1,create_time).setLong(2, notification.getId());
			query.setMaxResults(count);
			list = query.list();
	...
	public List<Notification> getNotificationsCollection(Long userId,int count) {
		String hql = "from Notification where recipientId=? and notificationType in (9,10,11,12,13) and status='enable' order by create_at desc";
		Session session = getSessionFactory().getCurrentSession();
		Query query = session.createQuery(hql).setLong(0, userId.longValue());
		query.setMaxResults(count);
		List<Notification> nList = query.list();
	...
	public Notification getNotificationsCollectionLastId(Long userId,int count) {
		String hql = "from Notification where recipientId=? and notificationType in (9,10,11,12,13) and status='enable' order by id desc";
		Session session = getSessionFactory().getCurrentSession();
		Query query = session.createQuery(hql).setLong(0, userId.longValue());
	...
	public List<Notification> getNotificationsByPageCollection(Long paramLong, int count, Long id, int flag) {
		...
		if(flag == 1){
			...
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String create_time = sdf.format(new Date(createTime.longValue()));
			String hql = "from Notification where recipientId=? and notificationType in (9,10,11,12,13) and create_at <= ? and id != ? and status='enable' order by create_at desc";
			Query query = session.createQuery(hql).setLong(0,paramLong).setString(1,create_time).setLong(2, notification.getId());
			query.setMaxResults(count);
			list = query.list();
			...
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String create_time = sdf.format(new Date(createTime.longValue()));
			String hql = "from Notification where recipientId=? and notificationType in (9,10,11,12,13) and create_at >= ? and id != ? and status='enable' order by create_at";
			Query query = session.createQuery(hql).setLong(0,paramLong).setString(1,create_time).setLong(2, notification.getId());
			query.setMaxResults(count);
			list = query.list();
			Collections.reverse(list);
	...
	public void enableNotification(int object_type, Long object_id) {
		String hql = "update Notification set status='enable' where objectType=? and objectId=?";
		Session session = getSessionFactory().getCurrentSession();
		Query query = session.createQuery(hql).setInteger(0,object_type).setLong(1, object_id);
		query.executeUpdate();
	}
	...
	public List<Notification> getNotificationsFollowers(String userids,int count) {
		String type = "3,4,14";
		String hql = "from Notification where 1=1 and status='enable' and senderId in ("+userids+") and notificationType in ("+type+") group by notificationType,objectType,objectId order by create_time desc";
		Session session = getSessionFactory().getCurrentSession();
	...
	public List<Notification> getNotificationsFollowers(String userids, int count, Long notification_id, int identify) {
		...
		if(identify == 1){
			...
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String create_time = sdf.format(new Date(createTime.longValue()));
			String hql = "from Notification where 1=1 and status='enable' and senderId in ("+userids+") and notificationType in ("+type+") and create_at <= ? and id != ? group by notificationType,objectType,objectId order by create_time desc";
			...
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String create_time = sdf.format(new Date(createTime.longValue()));
			String hql = "from Notification where senderId in ("+userids+")  and notificationType in ("+type+") and create_at >= ? and id != ? and status='enable' group by notificationType,objectType,objectId order by create_at";
			Query query = session.createQuery(hql).setString(0,create_time).setLong(1,notification_id);
			query.setMaxResults(count);
			list = query.list();
			Collections.reverse(list);
	...
	public Notification getNotificationsFollowers(String userids) {
		String type = "3,4,14";
		String hql = "from Notification where 1=1 and status='enable' and senderId in ("+userids+") and notificationType in ("+type+") group by notificationType,objectType,objectId order by id desc";
		Session session = getSessionFactory().getCurrentSession();
	...
	@Override
	public List<Notification> getNotificationList(int count) {
		String hql = "from Notification where 1=1 and status='enable' and recipientId=0 order by id desc";
		Session session = getSessionFactory().getCurrentSession();
		List<Notification> list = session.createQuery(hql).setMaxResults(count).list();
		return list;
	}
	...
	public int getNotificationCount() {
		String hql = "from Notification where 1=1 and status='enable' and recipientId=0";
		Session session = getSessionFactory().getCurrentSession();
		List<Notification> list = session.createQuery(hql).list();
		if(list != null){
			return list.size();
		}else{
			return 0;
	...
	@Override
	public List<Notification> getNotificationList(int count, Long max_id) {
		String hql = "from Notification where 1=1 and status='enable' and recipientId=0 and id < ? order by id desc";
		Session session = getSessionFactory().getCurrentSession();
		List<Notification> list = session.createQuery(hql).setLong(0, max_id).setMaxResults(count).list();
		return list;
	}
