...
import com.friend.rest.service.model.CollectionNotification;
import com.friend.rest.service.model.CommentModel;
import com.friend.rest.service.model.ContentModel;
import com.friend.rest.service.model.NotificationModel;
import com.friend.rest.service.model.StoryIntro;
import com.friend.rest.service.model.StoryIntroCollection;
...
public class NotificationServiceImpl implements NotificationService {
	...
	@Override
	public Response notificationInfo(Long loginUserid, JSONObject device,HttpServletRequest request) {
		...
		String client_id = device.getString("client_id");
		String device_type = device.getString("device_type");
		String device_token = null;
		if (device.containsKey("device_token")) {
			device_token = device.getString("device_token");
		}
		...
		if ((!Strings.isNullOrEmpty(client_id)) && (!Strings.isNullOrEmpty(device_type))
			...
			if (loginUserid != null && loginUserid > 0) {
				log.info("update pushNotification----->"+loginUserid);
				if (pn != null) {
					pn.setClientId(client_id);
					if(!Strings.isNullOrEmpty(device_token)){
	...
	@Override
	public Response clearNotification(JSONObject device) {
		...
		if ((!Strings.isNullOrEmpty(client_id)) && (!Strings.isNullOrEmpty(device_type))
			...
			returnJson.put("code", 1000);
			returnJson.put("error_message", "解绑成功");
			returnJson.put("data", "");
			return Response.status(Response.Status.OK).entity(returnJson).build();
		}else{
			returnJson.put("code", 1001);
			returnJson.put("error_message", "缺少必要参数");
			returnJson.put("data", "");
			return Response.status(Response.Status.OK).entity(returnJson).build();
		}
	...
	public JSONObject getNotifications(HttpServletRequest request) {
		...
		int num = notificationDao.getNotificationCount();
		if (Strings.isNullOrEmpty(countStr) 
				&& Strings.isNullOrEmpty(maxIdStr)) {
			nList = this.notificationDao.getNotificationList(count);
			...
			nList = this.notificationDao.getNotificationList(count);
				...
				&& !Strings.isNullOrEmpty(maxIdStr)) {
			nList = this.notificationDao.getNotificationList(count, Long.parseLong(maxIdStr));
		} else if (!Strings.isNullOrEmpty(countStr)
				&& !Strings.isNullOrEmpty(maxIdStr)) {
			count = Integer.parseInt(countStr);
			nList = this.notificationDao.getNotificationList(count, Long.parseLong(maxIdStr));
...
//		User author = null;//collection.getUser();// userDao.get(collection.getAuthorId());
...
//
//		UserCollection uc = null;//userCollectionDao.getUserCollectionByCollectionId(collection.getId(), loginUserid);
//		if (uc != null) {
...
//											ci.setCover_image(JSONObject.fromObject(collection.getCover_image()));
////											ci.setInfo(collection.getInfo());
//											User u = null;//userDao.get(collection.getUser().getId());
...
//											List<String> delArray = new ArrayList<String>();
//
//											int follow_collection_count = 0;//userCollectionDao
...
//		ci.setCover_image(JSONObject.fromObject(collection.getCover_image()));
////		ci.setInfo(collection.getInfo());
//		User u = null;//collection.getUser();// userDao.get(collection.getAuthorId());
//		JSONObject author = new JSONObject();
//		author.put("id", u.getId());
//		author.put("username", u.getUsername());
//		if (!Strings.isNullOrEmpty(u.getAvatarImage())) {
