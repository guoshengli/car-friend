...
import com.friend.rest.dao.FeatureCollectionDao;
import com.friend.rest.dao.FeatureUserDao;
import com.friend.rest.dao.InterestDao;
...
import com.friend.rest.service.model.CollectionDiscover;
import com.friend.rest.service.model.CollectionIntro;
import com.friend.rest.service.model.CollectionIntros;
import com.friend.rest.service.model.CollectionModel;
...
import com.friend.rest.service.model.UserFeatured;
...
//					 * true; break; } } }
//					 */
//					UserCollection uc = null;//userCollectionDao.getUserCollectionByCollectionId(c.getId(), loginUserid);
//					if (uc != null) {
...
//
//		User u = null;//c.getUser();// userDao.get(c.getAuthorId());
//		JSONObject author = new JSONObject();
//		author.put("id", u.getId());
//		author.put("username", u.getUsername());
//		if (!Strings.isNullOrEmpty(u.getAvatarImage())) {
...
//		}
//		
//		int count = 0;//collectionStoryDao.getStoriesByCount(c.getId());
...
//						slideModel.setId((Long) slide.getId());
//						slideModel.setType(slide.getType());
//						Collection collection = collectionDao.get(0);
//
//						CollectionIntro ci = new CollectionIntro();
////						ci.setId((Long) collection.getId());
////						ci.setCollection_name(collection.getCollectionName());
//						ci.setCover_image(JSONObject.fromObject(collection.getCover_image()));
////						ci.setInfo(collection.getInfo());
//						Set<User> uSet = null;//collection.getUsers();
//						if (uSet != null && uSet.size() > 0) {
...
//			Long since_id = Long.valueOf(Long.parseLong(sinceIdStr));
//
//			List<Collection> aList = null;//collectionDao.getCollectionBytype("activity", count, since_id, 1);
//			List<JSONObject> activityList = new ArrayList<JSONObject>();
//			if (aList != null && aList.size() > 0) {
//				for (Collection c : aList) {
//					JSONObject json = getCollectionInfo(c);
...
//				&& (!Strings.isNullOrEmpty(maxIdStr))) {
//			Long max_id = Long.valueOf(Long.parseLong(maxIdStr));
//			List<Collection> aList = null;//collectionDao.getCollectionBytype("activity", count, max_id, 2);
//			List<JSONObject> activityList = new ArrayList<JSONObject>();
//			if (aList != null && aList.size() > 0) {
//				for (Collection c : aList) {
//					JSONObject json = getCollectionInfo(c);
