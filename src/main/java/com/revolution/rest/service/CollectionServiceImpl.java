	     ...
	     if (cs != null) {
	       ...
	       Notification notification = this.notificationDao.getNotificationByAction(
			...
			if (conf.isDelete_story_from_collection_push()) {
				...
				String content = collection_user.getUsername() + " 删除你发布在 "+c.getCollectionName() +" 中的故事";
	       ...
	       Configuration conf = this.configurationDao.getConfByUserId(user.getId());
			if (conf.isDelete_story_from_collection_push()) {
				...
				if(audit == 1){
					...
					content = "站长 "+collection_user.getUsername() + " 接受你的投稿";
				}else if(audit == 2){
					content = "站长 "+collection_user.getUsername() + " 拒绝你的投稿";
				}
				JSONObject j = new JSONObject();
