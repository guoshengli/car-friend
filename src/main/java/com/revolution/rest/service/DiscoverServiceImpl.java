   ...
   public JSONObject getDiscover(Long loginUserid,HttpServletRequest request,String appVersion) { 
	 JSONObject discover = new JSONObject();
	  ...
	  List<SlideModel> slideModelList = new ArrayList<SlideModel>();
	       ...
	       for (Slide slide : slideList)
	       {
	         if (slide.getType().equals("url")) {
	           ...
	           if (!Strings.isNullOrEmpty(slide.getSlide_image())) {
	             slideModel.setSlide_image(JSONObject.fromObject(slide
	               .getSlide_image()));
	           }
	 
	           slideModelList.add(slideModel);
	         } else if (slide.getType().equals("story")) {
	           Story story = this.storyDao.getStoryByIdAndStatus(slide
	             .getReference_id(), "publish");
	           if (story != null) {
	             slideModel = new SlideModel();
	             slideModel.setId(slide.getId());
	             slideModel.setType(slide.getType());
	             JSONObject json = getStoryEventByLoginUser(
	               story, loginUserid);
	           ...
	           slideModel.setId((Long)slide.getId());
	           slideModel.setType(slide.getType());
	           UserParentModel userModel = getUserModel(slide.getReference_id(), 
	             loginUserid);
	   ...
	   List<FeatureCollection> fcList = featureCollectionDao.getFeatureCollectionByThree();
	   ...
	   discover.put("collection_recommend", recommandCollectionList);
...
//	    			 Set<User> uSet = c.getUsers();
	    			 ...
	    			 /*if(uSet != null && uSet.size() > 0){
	    				 Iterator<User> iter = uSet.iterator();
	    				 while(iter.hasNext()){
	    					 User u = iter.next();
	     ...
	     discover.put("collection_summary", summaryList);
	     
	     List<Object[]> hotList = userCollectionDao.getCollectionByHot();
	     ...
	     discover.put("collection_hot", hotCollectionList);
   ...
   public StoryModel getStoryModelByStoryLoginUser(Story story, Long loginUserid)
   {
     ...
     if ((seSet != null) && (seSet.size() > 0)) {
       for (StoryElement element : seSet) {
         content = JSONObject.fromObject(element
           .getContents());
   ...
   public JSONObject getStoryEventByLoginUser(Story story, Long loginUserid)
   {
     ...
     if ((story != null) && 
       (story.getStatus().equals("publish"))) {
       storyModel.setId((Long)story.getId());
     ...
     /*  String theme_color = null;
      ...
      /* if(loginUserid != null && loginUserid > 0){
    	   Republish repost = this.republishDao
    		         .getRepostByUserIdAndStoryId(loginUserid, story.getId());
	       if (repost != null)
	         storyModel.setRepost_by_current_user(true);
	       else {
	         storyModel.setRepost_by_current_user(false);
...
//         ci.setCover_image(JSONObject.fromObject(collection.getCover_image()));
         ci.setInfo(collection.getInfo());
         //Theme_color tc = (Theme_color)this.themeColorDao.get(collection.getTheme_id());
    ...
    // Follow loginFollow = this.followDao.getFollow(loginUserid, userId);
     ...
     if ((user.getUser_type().equals("normal")) || 
       ...
       userModel = new UserModel((Long)user.getId(), 
         ...
         coverImageJson,  repostCount, 
         storyCount, follower_Count, following_count, 
         website, followed_by_current_user, 
   ...
   public CollectionModel getCollectionModel(Collection collection) {
     ...
     if (collection != null) {
       ...
       if (!Strings.isNullOrEmpty(collection.getCover_image())) {
         collectionModel.setCover_image(JSONObject.fromObject(collection
           .getCover_image()));
       }
       if (!Strings.isNullOrEmpty(collection.getAvatar_image())) {
           collectionModel.setAvatar_image(JSONObject.fromObject(collection
             .getAvatar_image()));
         }
   ...
   public List<UserFeatured> getUserFeature() {
     ...
     if ((list != null) && (list.size() > 0)) {
       for (FeatureUser fu : list) {
         ...
         if (!Strings.isNullOrEmpty(user.getAvatarImage())) {
           JSONObject avatarJson = JSONObject.fromObject(user
             .getAvatarImage());
